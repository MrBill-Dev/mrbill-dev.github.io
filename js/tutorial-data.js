...
Object.assign(tutorialRepository, {
  ...
  work_git: {
    ...
    concept: "<b>日常循環</b>：改檔 → <code>git status</code> → <code>git add .</code> → <code>git commit -m \"說明\"</code> → <code>git push</code><br><b>分支</b>：新功能開 branch，完成後 Pull Request 請同事 review。<br><b>.gitignore</b>：排除 node_modules、.env 密碼檔。<br><b>衝突</b>：同一行被兩人改，手動合併後再 commit。<br><b>混合團隊</b>：若設計師／企劃沒裝 Git，見上一課「Git 09｜交接與二次備份」——你負責每次交出／收回都 commit 或 tag。",
...
const tutorialTrackRepository = {
  ...
  jq_06_ajax: { track: "jquery", title: "jQuery 06｜AJAX 與 API 串接", desc: "用 `$.get` / `$.ajax` 載入遠端資料。", concept: "讀 API 的核心是：請求、成功渲染、失敗提示。", practice: "按下按鈕抓取文章標題。", code: `<button id="load">載入資料</button><p id="out"></p>\n<script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>\n<script>\n$("#load").on("click", function(){\n  $.get("https://jsonplaceholder.typicode.com/posts/1")\n    .done(function(d){ $("#out").text(d.title); })\n    .fail(function(){ $("#out").text("載入失敗"); });\n});\n<\/script>`, tip: "一定要寫 fail 分支。", quiz: { q: "AJAX 失敗時應該？", opts: ["顯示可理解錯誤訊息", "什麼都不做", "直接重整頁面"], ans: 0 } },
  jq_07_form: { track: "jquery", title: "jQuery 07｜表單驗證實務", desc: "前端先做基本驗證，提高資料品質。", concept: "先擋明顯錯誤，再交由後端做最終驗證。", practice: "加入 email 格式檢查。", code: `<form id="f"><input id="email" placeholder="email"><button>送出</button></form><p id="msg"></p>\n<script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>\n<script>\n$("#f").on("submit", function(e){\n  e.preventDefault();\n  const v = $("#email").val();\n  const ok = /^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(v);\n  $("#msg").text(ok ? "格式正確" : "請輸入有效 email");\n});\n<\/script>`, tip: "規則寫清楚、錯誤訊息寫人話。", quiz: { q: "前端驗證可取代後端驗證嗎？", opts: ["不行", "可以完全取代", "只看專案大小"], ans: 0 } },
  ...
  git_09_mixed_team_handoff: {
    track: "git",
    title: "Git 09｜團隊有人沒裝 Git？交接與二次備份",
    desc: "分工時設計師／企劃常沒 Git。由會 Git 的人當版本管理員：交出前備份、收回後再備份。",
    concept: "<b>現實狀況</b>：你做完 layout 並用 Git 管理；設計師用 Figma／PS，回傳 zip 或改幾個 CSS，電腦可能沒裝 Git。<b>解法不是放棄版控</b>，而是指定一人（通常是你）當「版本管理員」——每次交接都多存一個 commit 或 tag。<br><br><b>五步交接節奏</b><br>① <b>第一次備份（交出前）</b>：layout 定稿 → <code>git commit -m \"feat: layout v1 — handoff to design\"</code>，建議加 tag <code>handoff-design-v1</code>；另 zip 一份 <code>site-layout-v1.zip</code> 當離線保險<br>② <b>交給設計師</b>：附簡短 README——可改 <code>css/</code>、<code>images/</code>；勿改 HTML 結構、路徑維持相對路徑、檔名小寫英文<br>③ <b>對方回傳</b>：zip／雲端連結皆可；請對方註明「改了哪些檔、日期、用的軟體」<br>④ <b>收回整合（勿直接覆蓋 main）</b>：解壓到暫存夾 → VS Code 比對或 <code>git diff</code> 看差異 → 開分支 <code>git switch -c intake/design-0529</code> 再逐檔合併<br>⑤ <b>第二次備份（驗收後）</b>：<code>git commit -m \"design: visual pass from 設計師名\"</code> → 本機預覽 OK 再 merge 回 main<br><br><b>沒 Git 的成員怎麼配合？</b><br>• <b>Zip 命名</b>：<code>project-layout-v1.zip</code> → <code>project-design-v2.zip</code>（版本號對齊 tag）<br>• <b>共用雲端</b>：Drive／OneDrive 開版本紀錄，但<b>仍以 Git commit 為準</b><br>• <b>限定可改範圍</b>：只動 theme.css 與 assets，結構由前端負責<br><br><b>溝通話術（可複製）</b><br>📤 交出：「layout v1 已在 Git tag <code>handoff-design-v1</code>，附 zip。請只改 css／圖，回傳請列改動檔名。」<br>📥 收回：「收到 design v2，我會比對後 commit；若跑版可回到 v1 tag。」<br>⚠️ 延期：「請勿在舊 zip 上覆蓋重傳，請用新檔名 v3，避免搞混版本。」",
    practice: "① 對專案做 layout commit + tag handoff-design-v1 ② 複製資料夾、改一個 css 當「設計師回傳」③ git diff 比對後做第二次 commit ④ 寫 3 行交接訊息給假想設計師",
    code: `<style>\n  .flow{font-family:system-ui;line-height:1.65;color:#334155;max-width:40em}\n  .step{border-left:4px solid #7c3aed;padding:10px 12px;margin:12px 0;background:#faf5ff;border-radius:0 10px 10px 0}\n  .step b{color:#5b21b6}\n  .msg{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px;font-size:13px;margin:10px 0}\n  pre{background:#0f172a;color:#a5f3fc;padding:12px;border-radius:10px;font-size:12px;line-height:1.7;overflow:auto}\n</style>\n<div class="flow">\n  <div class="step"><b>你（有 Git）</b> layout 完成 → commit + tag v1 → zip 備份</div>\n  <div class="step"><b>設計師（無 Git）</b> 改 css／圖 → 回傳 project-design-v2.zip + 改動清單</div>\n  <div class="step"><b>你（有 Git）</b> 比對 diff → 分支 intake → commit 第二次 → merge main</div>\n  <p class="msg"><strong>📤 交出範本</strong><br>Hi，layout v1 在 tag handoff-design-v1，zip 附檔。請只改 css/ 與 images/，回傳請用新檔名 v2 並列出改動檔。</p>\n  <p class="msg"><strong>📥 收回範本</strong><br>收到 v2，我今晚比對合併；若驗收有問題會 revert 到 v1，再請你修一版 v3。</p>\n  <pre>git commit -m "feat: layout v1 — handoff to design"\ngit tag handoff-design-v1\ngit switch -c intake/design-v2\n# 比對、合併設計師檔案\ngit commit -m "design: visual pass from 設計師名"\ngit switch main && git merge intake/design-v2</pre>\n</div>`,
    tip: "Git 是團隊保險箱：沒裝 Git 的人交檔案，裝 Git 的人負責存檔、比對、還原。",
    newbieTip: "寧可多 commit 一次，也不要設計師 zip 直接整包蓋掉而沒有上一版。",
    commonMistakes: "沒第一次備份就交出去；收回 zip 整包覆蓋 main、沒比對；設計師改完忘 commit，跑版時無法 revert。",
    submissionStandard: "1) 有 handoff tag 或 zip 命名規則\n2) 收回後有 intake 分支 + merge commit\n3) 能說出設計稿跑版如何 git revert 回 v1",
    quiz: { q: "設計師沒 Git、回傳 zip，你第一步該？", opts: ["確認已有 v1 commit/tag，再比對合併", "直接用 zip 覆蓋 main", "刪除 .git 重來"], ans: 0 }
  },
